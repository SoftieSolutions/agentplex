 ...
 * kept once more on the way to an agent -- including for the stale reason
 * `draining` that stack A adds, which will fail here until somebody decides
 * whether an agent should be told a machine is going away on purpose.
 */
...
export const serverShape = {
  ...
  staleReason: z
    ...
    .nullable()
    .describe('Why it is unreachable, as a word to branch on. Null while it is reachable.'),
  problem: z
    .string()
    .nullable()
