...
import { countsTowardAttention, type ServerConnectionReport } from './servers.js';

/**
 * Which stores a connected server leaves reachable.
 *
 ...
 * `countsTowardAttention` in `servers.ts` is that rule for one server, and
 * this is what it means for the stores.
 *
 ...
 *
 * The fleet state is what applies this: it has the sessions, and this says
 * which of them are on a machine anybody can still reach.
 */
