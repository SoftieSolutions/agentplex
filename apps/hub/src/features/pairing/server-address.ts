import { z } from 'zod';
import type { ServerAddress } from './pairing.js';

/**
 * The rules an address has to pass, and the two shapes this build writes.
 *
 * The parser a typed address goes through -- `serverAddressSchema` -- is the
 * feature's input and lives in `pairing.ts`. What is here is the rule set it
 * reads, the column's own parser with its one allowance, and the one function
 * that can produce an address nobody typed. The protocol is
 * transport-agnostic on purpose -- public DNS, a Tailscale name, an
 * SSH-tunnelled port are all the same to it -- so the rules below are about
 * the URL and never about the route.
 */
 ...
 *
 * One rule set, read by both parsers, so the loopback allowance is one named
 * branch rather than a second opinion about what an address is.
 */
export function addressProblem(
  text: string,
  allowLoopbackPlaintext: boolean,
): AddressProblem | null {
  let url: URL;
  try {
    url = new URL(text);
