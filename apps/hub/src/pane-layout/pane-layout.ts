 ...
 *
 * The node tree (`../catalogue/`) is the hub's to read: discovery inserts into
...
/**
 * The seam the client connection reads and writes through. Two functions and
 * no cache: a reply to whoever asked, with nothing to encode once.
 */
...
export function createPaneLayout({ database, clock }: PaneLayoutDependencies): PaneLayout {
  return {
    read: () => readPaneLayout(database),
    write: (layout) => writePaneLayout(database, layout, clock),
  };
}
