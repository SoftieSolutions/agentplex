...
import { createClientTickets, type ClientTickets, type IssuedTicket } from './client-tickets.js';
...
/**
 * Both halves of admission, over one ticket store.
 *
 * Nothing durable: a ticket outliving a restart would be a credential the hub
 * could not count the uses of, and the client holds the long-lived token it
 * can always exchange for another.
 */
export interface ClientAuth {
  answerTicketRequest(request: TicketRequest): TicketResponse;
  admitsUpgrade(url: string): boolean;
}

export function createClientAuth({ token, tokens, clock }: ClientAuthDependencies): ClientAuth {
  const tickets = createClientTickets({ tokens, clock });
  return {
    answerTicketRequest: (request) => answerTicketRequest(request, { token, tickets }),
    admitsUpgrade: (url) => admitsUpgrade(url, tickets),
  };
}
