 ...
 * way `client-auth.ts` decides the ticket exchange: `http/routes.ts` turns an
 * answer into HTTP and decides nothing. The two rules that carry the weight are the
 ...
 * comparison made before decoding. `http/routes.ts` has already collapsed `..` through
...
export interface WebDependencies {
  /**
   * The built PWA, on whatever disk it turned out to be on. A dependency
   * rather than a directory this file opens: where the client lives is a fact
   * about how agentplex was installed -- a workspace build, a layer in the
   * image, a published package -- and `main.ts` is the only thing that knows
   * which of them this is.
   */
  readonly files: WebAssetFileSystem;
  readonly logger: Logger;
}

export interface Web {
  /** Answers a request for the client, or refuses it. */
  answer(request: WebAssetRequest): Promise<WebAssetAnswer>;
  /**
   ...
   */
  reportBuild(): Promise<void>;
}

export function createWeb({ files, logger }: WebDependencies): Web {
  return {
    answer: (request) => answerWebAssetRequest(request, files),

    async reportBuild(): Promise<void> {
      try {
        const shell = await files.read(SHELL_FILE);
        if (shell === null) {
          logger.warn('no client build to serve', { from: files.root });
          return;
        }
        logger.info('serving the client', { from: files.root });
      } catch (error) {
        // A web root that cannot be read at all. Still not a reason not to
        // start: the same 503 covers it, and this is the line that explains it.
        logger.warn('client build unreadable', { from: files.root, error: String(error) });
      }
    },
  };
}
