...
/**
 * The pairing feature over the same schema. The real one: what a handshake
 * records is half of what this suite asserts on, and a stand-in would be the
 * test writing down the answer it then reads back.
 *
 * It takes the loop's own clock, because the row a handshake writes is stamped
 * with it: a suite that moved time for the connection and not for the table
 * would read back a `lastConnectedAt` from a different week.
 */
function pairing(now: () => number = () => START): Pairing {
...
let running: DialLoop | null = null;
...
function start(
  ...
  const connection = startDialLoop(registration, {
    pairing: pairing(options.now ?? (() => START)),
