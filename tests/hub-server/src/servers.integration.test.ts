...
/**
 * The pairing feature over the same schema, which is how the servers feature
 * reads the table now. The real one rather than a stand-in: what the
 * supervisor does with a revoked pairing is the point of half this suite, and
 * a hand-written `listServers` would be the test agreeing with itself.
 */
