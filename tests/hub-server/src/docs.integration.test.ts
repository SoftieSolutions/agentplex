...
async function start(): Promise<Harness> {
  ...
  const docs = createDocs({ database, ids, clock, state, projects, connections, logger });
  ...
  const clients = createClients({
    ...
    sessions: createSessions({
      ...
      projects,
      connections,
      ids: { newId: () => 'unused-start' },
      logger,
    }),
    // Nothing here is about terminals either, and the relay's own fake is
