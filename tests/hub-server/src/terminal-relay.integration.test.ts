...
function serveMachine(machine: Machine): DialResult {
  ...
  const files: ProviderFiles = {
    ...
    stat: (path) => createFakeProviderFiles({ files: machine.sessionFiles }).stat(path),
  };
  ...
  serveServerEnd(serverEnd, {
    ...
    sessions: createSessionController({
      ...
      // No roots, which is the default a server ships with: nothing in this
...
async function start(
  ...
  const sessions = createSessions({
    state,
    // A fake project table: every start here names a store and a machine, and
    // the rows are the project suites' subject rather than this one's.
