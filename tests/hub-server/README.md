...
by relative path, the way `apps/install` composes their built output by
path, and they live here rather than in either app so that neither app's
source reaches the other's. `pnpm lint` allows the crossing in this
